...
def grade_statistics():
    ...
    try:
        with open(FILE, "r") as file:
            ...
            for row in reader:
                ...
                student = format_student(row)
                grade = student["grade"]

                if grade not in grade_data:
                    grade_data[grade] = []

                grade_data[grade].append(student["name"])
    ...
    total_students = sum(len(names) for names in grade_data.values())

    print("\n--- GRADE STATISTICS ---")

    for grade, students in sorted(grade_data.items()):
        count = len(students)
